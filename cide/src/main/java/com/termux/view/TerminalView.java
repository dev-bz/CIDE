...
public final class TerminalView extends View {
        ...
        mGestureRecognizer = new GestureAndScaleRecognizer(context, new GestureAndScaleRecognizer.Listener() {
            ...
            public boolean onSingleTapUp(MotionEvent e) {
                ...
                if (!mEmulator.isMouseTrackingActive()) {
                    if (!e_isFromSource(e, InputDevice.SOURCE_MOUSE)) {
            ...
            public boolean onScroll(MotionEvent e2, float distanceX, float distanceY) {
                ...
                if (mEmulator.isMouseTrackingActive() && e_isFromSource(e2, InputDevice.SOURCE_MOUSE)) {
    ...
    private boolean handleVirtualKeys(int keyCode, KeyEvent event, boolean down) {
        ...
        if (inputDevice != null && inputDevice.getKeyboardType() == InputDevice.KEYBOARD_TYPE_ALPHABETIC) {
            ...
            if (LOG_KEY_EVENTS)
                Log.i(EmulatorDebug.LOG_TAG, "handleVirtualKeys(down=" + down + ") taking ctrl event");
            mVirtualControlKeyDown = down;
            return true;
        } else if (keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
            if (LOG_KEY_EVENTS)
                Log.i(EmulatorDebug.LOG_TAG, "handleVirtualKeys(down=" + down + ") taking Fn event");
    ...
    public void checkForFontAndColors() {
        try {
            File fontFile = new File(getContext().getFilesDir(), "home/.termux/font.ttf");
            File colorsFile = new File(getContext().getFilesDir(), "home/.termux/colors.properties");
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        return new BaseInputConnection(this, true) {
            ...
            public int getCursorCapsMode(int reqModes) {
                if (LOG_KEY_EVENTS)
                    Log.i(EmulatorDebug.LOG_TAG, "IME: getCursorCapsMode(" + reqModes + ")");
                int mode = 0;
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        if (mEmulator != null && e_isFromSource(event, InputDevice.SOURCE_MOUSE) && event.getAction() == MotionEvent.ACTION_SCROLL) {
    ...
    private boolean e_isFromSource(MotionEvent e, int sourceMouse) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            return e.isFromSource(sourceMouse);
        } else return (e.getSource() & sourceMouse) == sourceMouse;
    }

    private boolean e_isButtonPressed(MotionEvent e, int button) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return e.isButtonPressed(button);
        } else {
            if (button == 0) {
                return false;
            }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        if (mIsSelectingText) {
            ...
            return true;
        } else if (e_isFromSource(ev, InputDevice.SOURCE_MOUSE)) {
            if (e_isButtonPressed(ev, MotionEvent.BUTTON_SECONDARY)) {
                if (action == MotionEvent.ACTION_DOWN) showContextMenu();
                return true;
            } else if (e_isButtonPressed(ev, MotionEvent.BUTTON_TERTIARY)) {
    ...
    public void toggleSelectingText(MotionEvent ev) {
        ...
        /*if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
          ...
          mLeftSelectionHandle = (BitmapDrawable) getResources().getDrawable(R.drawable.text_select_handle_left_material);
          mRightSelectionHandle = (BitmapDrawable) getResources().getDrawable(R.drawable.text_select_handle_right_material);
        }*/
                int[] n = {android.R.attr.textSelectHandleLeft, android.R.attr.textSelectHandleRight};
                TypedArray t = getContext().obtainStyledAttributes(n);
                mLeftSelectionHandle = (BitmapDrawable) t.getDrawable(t.getIndex(0));
                mRightSelectionHandle = (BitmapDrawable) t.getDrawable(t.getIndex(1));
                t.recycle();
            }
            int cx = (int) (ev.getX() / mRenderer.mFontWidth);
            final boolean eventFromMouse = e_isFromSource(ev, InputDevice.SOURCE_MOUSE);
            // Offset for finger:
            ...
            final ActionMode.Callback callback = new ActionMode.Callback() {
                ...
                public boolean onCreateActionMode(ActionMode mode, Menu menu) {
                    ...
                    ClipboardManager clipboard = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
                    menu.add(Menu.NONE, 1, Menu.NONE, R.string.copy_text).setIcon(styledAttributes.getResourceId(styledAttributes.getIndex(0), 0)).setShowAsAction(show);
                    menu.add(Menu.NONE, 2, Menu.NONE, R.string.paste_text).setIcon(styledAttributes.getResourceId(styledAttributes.getIndex(1), 0)).setEnabled(clipboard.hasPrimaryClip()).setShowAsAction(show);
